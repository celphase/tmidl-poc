...
typedef pcc_auxil_o *pcc_auxil_t;
...
static pcc_thunk_chunk_t *pcc_evaluate_rule_tmidl(pcc_context_t *ctx) {
    ...
    chunk->pos = ctx->cur;
    PCC_DEBUG(PCC_DBG_EVALUATE, "tmidl", ctx->level, chunk->pos, (ctx->buffer.buf + chunk->pos), (ctx->buffer.len - chunk->pos));
    ctx->level++;
...
L0000:;
    ctx->level--;
    PCC_DEBUG(PCC_DBG_NOMATCH, "tmidl", ctx->level, chunk->pos, (ctx->buffer.buf + chunk->pos), (ctx->cur - chunk->pos));
...
int pcc_parse(pcc_context_t *ctx, int *ret) {
    ...
    if (pcc_apply_rule(ctx, pcc_evaluate_rule_tmidl, &thunks, ret))
